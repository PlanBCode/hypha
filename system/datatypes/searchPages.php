...
*/
	class searchPage extends Page {
		...
		$responce = hyphaSearchPattern($pattern,$includePrivatePages,$context);
		//$hyphaHtml->writeToElement('pagename', __('searchresult')); uitgezet bz
		if ($responce) {
			$this->html->writeToElement('main', $responce);
		}
		else {
	...
	global $uiLangList;
	global $isoLangList;
	$responce = "Error Internal error:illegal call to search, specify search type";
		...
		if ($_pagePrivate === "off" || $bprivate )
		 {
			foreach($page->getElementsByTagName('language') as $language)
			{
				...
				$_htmlResult .= searchSource($_fileName,$spattern,$_pageType,$_pageLanguageId,$_pageLanguageName,$context);
			} // "   EINDE language ==============================\n";
		}
	} //"EINDE page ==================================\n";
}
...
if (!($pos === false))
{
	$result =  formatsearchresult($mystring,$pattern,$pos,$context);
}
return $result;
}
...
foreach ($texts->getElementsByTagName('article') as $article)
{
	foreach ($article->getElementsByTagName('content') as $content)
	{
		foreach ($content->getElementsByTagName('text') as $textContent)
		{
			...
			$searchResult = searchAction($lines,$pattern,$icontextlength);
		}
	}
}
return $searchResult;
...
switch ($pageType)
{
	case 'peer_reviewed_article':
		$lhtmlResult = searchPeerReviewedArticle($filename,$pattern,$_pageLanguageId,$icontextlength);
	break;
	case 'textpage':
		$lhtmlResult = searchTextPage($filename,$pattern,$_pageLanguageId,$icontextlength);
	break;
	default: $lhtmlResult = "";
}
if (!$lhtmlResult == "")
{
	...
	$lhtmlPage = render($lhtmlResult,$_pageLanguageId,$_pageLanguageName);
	// echo "\n" . $lhtmlPage . "\n";
}
return $lhtmlPage;
}
