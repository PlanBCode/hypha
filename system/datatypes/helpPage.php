...
//			else return notify('error', __('login-to-view'));
			...
			switch ($this->getArg(0)) {
				//help/user/<username>
				case 'userhelp': $this->showhelpuser($this->getArg(1)); break; // get info about user identiefied by name in arg(1)
				//help/pagenamehelp/<pagetype>
				case 'pagenamehelp': $this->pagenamehelp($this->getArg(1)); break; //get info about a specific pagetype
				case 'menuhelp':     $this->showhelpMenu($this->getArg(1)); break;
				case 'helpuser':     $this->showhelpuser($this->getArg(1)); break;
				case 'button':       $this->infobutton($this->getArg(1)); break;
				case 'help':         $this->helpinfo($this->getArg(1),'nl'); break;
				default:             $this->showhelpindex($this->getArg(1));break;
			}
		}
...
function gethelpbuttoninfo($button,$_language){
 return "Help voor" . $button . " in de taal ". $language;
}

function helpinfo($subject,$language){
$info =  "HELPPAGES.PHP HELPINFO: argument subject ". $subject . " in de taal ". $language . "<br>\n";
//	$info = hypha_searchhelp($subject, $language);
	return $info;
}
