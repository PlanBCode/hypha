...
<script>
	...
	function newPage() {
		...
		html+= '<tr><th><?=__('type')?></th><td><select id="newPageType" name="newPageType">' + '<?php foreach($types as $type => $pagename) echo '<option value="'.$type.'"'.($type=='textPage' ? 'selected="selected"' : '').'>'.$pagename.'</option>'; ?>' + '</select></td></tr>';
